...
import com.binhanh.protocol.serializer.model.KeyIntegerInterface;
...
public class SparseArraySerializer extends TypeSerializer{
	...
	public Object deserialize(Class<?> cls, ExtendedByteBuffer buffer, Annotation... annotations) {
		...
		for (int i = 0; i < size; i++) {
		    ...
		    if(o instanceof KeyIntegerInterface){
                map.put(((KeyIntegerInterface)o).getKey(), o);
            }else{
		        throw new IllegalArgumentException("Đối tượng không kế thừa KeyIntegerInterface: " + o.getClass().getName());
            }
		}
		return map;
	}
